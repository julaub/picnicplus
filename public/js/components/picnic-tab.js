...
export const initPicnicTab = (containerId) => {
    ...
    const renderDashboard = () => {
        ...
        const eventName = state.picnicDetails.name || 'Your Event';
        ...
        header.innerHTML = `
            <div class="pp-header-row">
                <div>
                    <div class="pp-subtitle" style="color:var(--green-700);text-transform:uppercase;letter-spacing:.06em;font-size:11px;font-weight:700;">Your Event</div>
        ...
        hero.innerHTML = `
            <div class="inner">
                <div class="pp-event-status"><span class="dot"></span> Confirmed</div>
                <div class="pp-event-name">${eventName}</div>
                <div class="pp-event-meta">${dateLabel}</div>
                <div class="pp-event-countdown">
                    <div class="pp-count-cell"><div class="pp-count-num">${String(days).padStart(2,'0')}</div><div class="pp-count-label">Days</div></div>
                    <div class="pp-count-cell"><div class="pp-count-num">${String(hours).padStart(2,'0')}</div><div class="pp-count-label">Hours</div></div>
                    <div class="pp-count-cell"><div class="pp-count-num">${String(mins).padStart(2,'0')}</div><div class="pp-count-label">Min</div></div>
                    <div class="pp-count-cell"><div class="pp-count-num">${String(secs).padStart(2,'0')}</div><div class="pp-count-label">Sec</div></div>
                </div>
            </div>`;
        ...
        locTile.innerHTML = `
            ...
            <div class="pp-tile-body">
                <div class="pp-tile-label">Location</div>
                <div class="pp-tile-value" id="event-location-name">Loading…</div>
            </div>
        ...
        statRow.innerHTML = `
            <div class="pp-stat"><div class="pp-stat-num">${going}</div><div class="pp-stat-label">Guests</div></div>
            <div class="pp-stat"><div class="pp-stat-num">${potluckCount}</div><div class="pp-stat-label">Items</div></div>
            <div class="pp-stat"><div class="pp-stat-num">${claimedCount}</div><div class="pp-stat-label">Claimed</div></div>`;
        ...
        dateSectionTitle.innerHTML = '<span>Date Options</span>';
        ...
        if (!state.dates || state.dates.length === 0) {
            ...
            empty.textContent = 'No dates proposed yet.';
            ...
            state.dates.forEach(date => {
                ...
                tile.innerHTML = `
                    ...
                    <div style="flex:1;min-width:0;">
                        ...
                        <div style="font-size:12px;color:var(--ink-500);margin-top:2px;">${voteCount} vote${voteCount !== 1 ? 's' : ''}${voters ? ' · ' + voters : ''}</div>
                    </div>`;
                ...
                voteBtn.textContent = myVote ? '✓ Voted' : 'Vote';
        ...
        cta.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg> Share event link`;
        cta.addEventListener('click', () => {
            if (navigator.share) {
                ...
                navigator.clipboard.writeText(shareUrl).then(() => {
                    ...
                    setTimeout(() => { cta.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg> Share event link`; }, 2000);
                });
            }
        });
