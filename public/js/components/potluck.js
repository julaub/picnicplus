...
const CATS = [
    { k: 'all', label: 'All' },
    { k: 'needed', label: 'Needed' },
    { k: 'covered', label: 'Covered' },
];
...
export const initPotluck = (containerId) => {
    ...
    const removeItem = async (itemId) => {
        if (!state.currentUser) return;
        if (confirm('Remove this item?')) await removePotluckItemApi(itemId);
    };
    ...
    const renderPotluck = () => {
        ...
        header.innerHTML = `
            <div>
                <div class="pp-subtitle" style="color:var(--green-700);text-transform:uppercase;letter-spacing:.06em;font-size:11px;font-weight:700;">Food, Drinks & Items</div>
                <h1 class="pp-title">Potluck list</h1>
                <div class="pp-subtitle" style="margin-top:4px;">Claim what you're bringing</div>
            </div>`;
        ...
        if (filtered.length === 0 && total === 0) {
            card.innerHTML = `<div style="padding:24px 16px;text-align:center;color:var(--ink-400);font-size:14px;">No items yet — suggest the first one!</div>`;
        } else if (filtered.length === 0) {
            card.innerHTML = `<div style="padding:20px 16px;text-align:center;color:var(--ink-400);font-size:14px;">No items in this category.</div>`;
        } else {
            filtered.forEach(item => {
                ...
                body.innerHTML = `
                    ...
                    <div class="pp-item-meta">${totalClaimed}/${item.quantity} · ${isCovered && claimedByNames ? 'brought by ' + claimedByNames : 'unclaimed'}</div>`;
                ...
                if (!isCovered && state.currentUser) {
                    ...
                    claimBtn.title = 'Join event to claim';
        ...
        if (state.currentUser) {
            ...
            addRow.innerHTML = `
                <input type="text" id="new-item-input" placeholder="Suggest an item…" style="flex:1;height:44px;border-radius:999px;padding:0 16px;font-size:14px;font-weight:500;background:#fff;border:1.5px dashed var(--ink-300);color:var(--ink-900);font-family:inherit;outline:none;">
            ...
            hint.innerHTML = '<span>ℹ️</span><span>Join the event to claim items or suggest new ones.</span>';
